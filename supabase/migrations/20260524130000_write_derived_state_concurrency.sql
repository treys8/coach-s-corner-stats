-- Concurrency guard for write_derived_state.
--
-- Before: two devices scoring the same game (head coach on tablet, assistant
-- on a phone, etc.) could each insert their own events via apply_game_events
-- (safe — idempotent on client_event_id + monotonic sequence_number) and
-- then each call write_derived_state with a derived payload computed from
-- THEIR local view of game_events. The local views diverged because each
-- device only saw its own apply_game_events return value, not the peer's.
-- Last write wins → the derived state row (game_live_state, at_bats,
-- games.{score,status,result}) reflected only one coach's events even
-- though both were persisted.
--
-- After: the RPC gains an optional p_expected_last_seq parameter — the
-- max(sequence_number) of game_events the caller's state was computed
-- against. The function takes a row-level lock on games, reads the actual
-- current max(sequence_number), and aborts with ERRCODE='40001'
-- (serialization_failure) on mismatch. The caller (rederive in
-- src/lib/scoring/server.ts) catches that code, re-fetches events,
-- re-replays, and retries with the fresh expected_last_seq.
--
-- The check is opt-in (p_expected_last_seq IS NULL bypasses it) so any
-- callers not yet wired through the retry harness keep working unchanged.
-- The TS code always passes the value once this migration ships.
--
-- DROP + CREATE is required because we're changing the function's
-- signature. The new GRANT mirrors the old one. The body below is
-- otherwise identical to 20260519120000_write_derived_state_rpc.sql.

DROP FUNCTION IF EXISTS public.write_derived_state(UUID, JSONB);
