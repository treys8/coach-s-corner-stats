  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        .from("schools")
        .select("id, slug, name, short_name, logo_url, primary_color, secondary_color, is_discoverable, public_scores_enabled, association, classification, division, timezone")
        .eq("slug", schoolSlug)
        .maybeSingle();
      if (!active) return;
      if (error || !data) {
