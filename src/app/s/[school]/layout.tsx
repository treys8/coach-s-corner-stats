  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        .from("schools")
        .select("id, slug, name, short_name, logo_url, primary_color, secondary_color, is_discoverable, public_scores_enabled, is_public_roster, association, classification, division, region, timezone")
        .eq("slug", schoolSlug)
        .maybeSingle();
      if (!active) return;
      if (error || !data) {
